        ...
        <div id="layoutSidenav_content">
            <div class="container-lg">
                <div class="header_part">
                    <h5><a href="{{ route('all_category') }}">Category Manage</a>
                    </h5>
                    <div class="right_content">
                        <a href="{{ route('catagory_pdf') }}"><i class="fa-solid fa-download"></i>
                            Download Pdf</a>
                        <a href="{{ route('showCategore') }}"><i class="fa-solid fa-plus"></i>Create Category</a>
                        <form action="{{ route('category_filter') }}"
                            class="d-none d-md-inline-block form-inline ms-auto me-0 me-md-3 my-2 my-md-0" method="get">
                            <div class="input-group">
                                <input class="form-control ml-2" type="text" placeholder="Search for..."
                                ...
                                <tr>
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="ms-3">
                                                <p class="fw-bold mb-1 text-capitalize">{{ $cate->name }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <p class="fw-normal mb-1">{{ $cate->slug }}</p>
                                    </td>
                                    <td>
                                        @if ($cate->status === 'active')
                                            <strong>Active</strong>
                                        @elseif($cate->status === 'deactive')
                                            <strong>Deactive</strong>
                                        @endif
                                    </td>
                                    <td>
                                        <p class="fw-normal mb-1">{{ $cate->created_at->format('d/m/Y') }}</p>
                                    </td>
                                    <td class="edit_delete">
                                        <a href="{{ route('edit_category', $cate->id) }}"><i class="fas fa-edit"></i></a>

                                        @if (Auth::user()->hasRole('admin'))
                                            <button type="button" class="btn btn-link p-0 text-danger btn-delete"
                                                data-url="{{ route('delete_category', $cate->id) }}"
                                                data-name="{{ $cate->name }}" data-bs-toggle="modal"
