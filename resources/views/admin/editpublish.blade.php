...
@section('content')
                        ...
                        <div class="col-xl">
                            <div class="card mb-4">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                    <h5 class="mb-6 pt-6 pb-6">Edit Published Post</h5>
                                ...
                                @endif
                                <div class="card-body">
                                    <form action="{{ route('update_publish', ['id' => $post->id]) }}" method="POST">
                                        @csrf
                                        <div class="mb-3">
                                            @error('publish')
                                                <div
    ...
    margin: 10px 0px 10px 0px;">
                                            ...
                                            <label for="exampleFormControlSelect1" class="form-label">Post Publish</label>
                                            <select
                                            ...
                                            @enderror"
                                                ...
                                                name="publish">
                                                <option value="yes" {{ $post->publish == 'yes' ? 'selected' : '' }}>
                                                    Yes</option>
                                                <option value="no" {{ $post->publish == 'no' ? 'selected' : '' }}>
                                                    No</option>
                                            </select>
                                        </div>
                                        <button type="submit" class="btn btn-primary mb-2 mt-3">Update Publisation</button>
                                    </form>
                                </div>
                            </div>
                        </div>
